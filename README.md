...
- **SM2 非对称加密** - 256-bit 椭圆曲线数字签名与公钥加密，支持 DER 编码
- **SM3 哈希算法** - 国密哈希与 HMAC，用于审计日志完整性校验
- **SM4 对称加密** - 128-bit 对称加密 (CBC/ECB/CTR)，支持 PDF 文件级加密
- **等保三级合规** - 满足《密码法》与等保三级要求

### 📋 审计日志

- **防篡改日志** - 基于 SM3 哈希链的区块链式审计记录
- **SQLite 存储** - better-sqlite3 同步写入，保证原子性
- **触发器保护** - SQLite 触发器禁止 UPDATE/DELETE 操作
- **完整性验证** - 遍历全量记录重新计算哈希链，检测任何篡改
- **多维度查询** - 按动作类型、时间范围、用户、安全等级过滤
- **日志导出** - 支持 JSON/CSV 格式导出

### 🔤 字体管理

- **中文字体支持** - 思源宋体 (Source Han Serif)、阿里巴巴普惠体 (Alibaba PuHuiTi)
- **字体发现** - 自动扫描系统与内置字体目录
- **字体注册** - 跨平台字体注册 (macOS/Linux/Windows)
- **完整性校验** - SHA-256 字体文件校验
- **PDF 嵌入** - 字体文件直接嵌入 PDF 文档

### 📑 PDF/A 转换与验证

- **PDF/A 转换** - Ghostscript 引擎，支持 PDF/A-1b/2b/3b 合规级别
- **PDF/A 验证** - VeraPDF 引擎验证 PDF/A 合规性
- **环境检测** - 自动检测 Ghostscript/VeraPDF 可用性与版本
- **降级方案** - pdf-lib 降级转换（仅 XMP 元数据 + OutputIntent）
...
| **PDF 修复** | 修复损坏的 PDF 文件 |
| **OCR 文字识别** | PaddleOCR (主) + Tesseract.js (降级) 中英文文字识别，生成可搜索 PDF |
...
| **国密算法** | sm-crypto | 0.4.x | SM2/SM3/4 国密算法套件 (等保三级) |
| **审计数据库** | better-sqlite3 | 12.x | SQLite 原生绑定 (审计日志存储) |
| **Canvas** | canvas | 3.x | Node.js Canvas (PDF 渲染辅助) |
| **OCR (主)** | @paddleocr/paddleocr-js | 0.4.x | PaddleOCR JS 引擎 (中英文识别) |
| **OCR (降级)** | Tesseract.js | 7.x | 纯 JavaScript OCR 引擎 |
| **JS 沙箱** | quickjs-emscripten | 0.32.x | QuickJS 沙箱执行引擎 |
...
- **Ghostscript** - PDF 压缩、PDF/A 转换与处理
- **QPDF** - PDF 加密/解密与结构优化
- **VeraPDF** - PDF/A 合规性验证
- **LibreOffice** - 文档格式转换
...
```bash
# 克隆仓库
git clone https://github.com/zhaohai666/VerityPDF.git
cd VerityPDF
...
│   ├── crypto/                  # 国密算法服务 (SM2/SM3/SM4)
│   ├── diff/                    # PDF 对比
│   ├── encryption/              # 加密解密 (QPDF)
│   ├── export/                  # PDF 导出
│   ├── font/                    # 中文字体管理 (思源宋体/阿里巴巴普惠体)
│   ├── form/                    # 表单处理
│   ├── nup/                     # N-up 排版
│   ├── overlay/                 # PDF 叠加
│   ├── pdf/                     # 内容流编辑
│   ├── pdfa/                    # PDF/A 转换与验证 (Ghostscript/VeraPDF)
│   ├── redact/                  # 敏感信息涂黑
│   ├── redaction/               # 密文修订
│   ├── repair/                  # PDF 修复
│   ├── resize/                  # 页面尺寸调整
│   ├── script/                  # QuickJS 脚本执行引擎
│   ├── services/                # 通用服务层
│   ├── show-js/                 # JavaScript 调试
│   ├── signature/               # 数字签名
│   └── utils/                   # 工具函数
│
├── src/                         # 渲染进程 (React)
...
│   ├── opencv/                  # OpenCV WASM 模块
│   └── standard_fonts/          # 标准字体文件
│
├── resources/                   # 打包资源
│   ├── fonts/                   # 中文字体 (思源宋体/阿里巴巴普惠体)
│   └── icons/                   # 应用图标 (macOS/Windows/Linux)
│
├── vendor/                      # 第三方二进制依赖
│   ├── gs/                      # Ghostscript (PDF 压缩/PDF/A 转换)
│   └── qpdf/                    # QPDF (PDF 加密/解密)
│
├── test/                        # 测试资源
├── package.json                 # 项目配置
...
│   ├── 格式转换 (LibreOffice)
│   ├── 国密算法服务 (SM2/SM3/SM4)
│   ├── 审计日志服务 (SQLite + SM3 哈希链)
│   ├── 字体管理服务 (思源宋体/阿里巴巴普惠体)
│   ├── PDF/A 转换与验证 (Ghostscript/VeraPDF)
│   ├── OCR 服务 (PaddleOCR/Tesseract.js)
│   ├── 原生菜单 & 快捷键
│   └── IPC 调度 (registerIpcHandler)
│
├── Preload Script (安全桥接层)
│   └── contextBridge - 暴露受限 verityAPI
│
...
- **国密算法合规** - SM2/SM3/SM4 国密算法套件，满足等保三级与《密码法》要求
- **审计日志防篡改** - SM3 哈希链完整性保护，SQLite 触发器禁止修改/删除
- **IPC 类型安全** - `registerIpcHandler<Request, Response>` 泛型封装，编译期类型检查
